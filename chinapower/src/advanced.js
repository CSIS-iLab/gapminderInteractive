...
function introWatchStepChange() {
  ...
  introJs.onchange(function (targetElement) {
    ...
    if (currentStep == 1 || currentStep == 5 || currentStep == 10) {
      document
        .querySelector(".introjs-tooltipReferenceLayer")
        .classList.add("intro-circleSelectRef");
    } else if (currentStep != 0) {
      document
        .querySelector(".introjs-tooltipReferenceLayer")
        .classList.remove("intro-circleSelectRef");
    }
    ...
    if (args.hide) {
      args.hide.forEach(function (country) {
        let checkbox = document.querySelector(
          'input[name="country"][data-iso="' + country + '"]'
        );
...
function setupHints() {
  ...
  axes.forEach(function (axis) {
    ...
    if (axis == "c") {
      document
        .querySelector(".filter-axis-c")
        .setAttribute("data-hintPosition", "middle-right");
    }
